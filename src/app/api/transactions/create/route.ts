import type { NextRequest } from 'next/server';
import prisma from '../../../../../lib/prisma';
import { makeid } from '@/lib/utils';

const MIDTRANS_URL = process.env.NEXT_PUBLIC_MIDTRANS_TRANSACTION_URL ?? '';
const MIDTRANS_SERVER_KEY = process.env.NEXT_PUBLIC_MIDTRANS_AUTH_KEY ?? '';

export async function POST(request: NextRequest) {
	const body = await request.json();

	try {
		// Validasi Midtrans configuration
		if (!MIDTRANS_URL || !MIDTRANS_SERVER_KEY) {
			console.error('Midtrans configuration missing');
			return Response.json({ error: 'Payment gateway configuration error' }, { status: 500 });
		}

		const transaction = await prisma.ticket.create({
			data: {
				bookingDate: body.bookingDate,
				price: body.price,
				status: 'PENDING',
				customerId: body.customerId,
				flightId: body.flightId,
				seatId: body.seatId,
				code: `TRX${makeid(7)}`,
			},
		});

		await prisma.flightSeat.update({
			where: {
				id: transaction.seatId,
			},
			data: {
				isBooked: true,
			},
		});

		// handle untuk midtrans
		// Encode Server Key to Base64
		const authString = Buffer.from(`${MIDTRANS_SERVER_KEY}:`).toString('base64');

		const parameter = {
			transaction_details: {
				order_id: transaction.id,
				gross_amount: Number(body.price),
			},
			credit_card: {
				secure: true,
			},
			item_details: [
				{
					id: body.flightId,
					price: Number(body.price),
					quantity: 1,
					name: `Tiket Pesawat ${body.departureCityCode} - ${body.destinationCityCode}`,
				},
			],
		};

		console.log('Calling Midtrans API:', MIDTRANS_URL);
		console.log('Transaction ID:', transaction.id);
		console.log('Price:', body.price);

		const resMidtrans = await fetch(MIDTRANS_URL, {
			method: 'POST',
			headers: {
				accept: 'application/json',
				'content-type': 'application/json',
				authorization: `Basic ${authString}`,
			},
			body: JSON.stringify(parameter),
		});

		const midtrans = await resMidtrans.json();

		console.log('Midtrans Response Status:', resMidtrans.status);
		console.log('Midtrans Response:', midtrans);

		// Check Midtrans response
		if (!resMidtrans.ok) {
			console.error('Midtrans API Error:', midtrans);
			console.error('Response Status:', resMidtrans.status);
			throw new Error(midtrans.error_messages?.[0] || 'Midtrans API error');
		}

		await prisma.ticket.update({
			where: {
				id: transaction.id,
			},
			data: {
				tokenMidtrans: midtrans.token,
			},
		});
		// handle untuk midtrans

		console.log('Transaction created successfully:', transaction.id);

		return Response.json({ midtrans, transaction_id: transaction.id });
	} catch (error) {
		console.error('Transaction API Error:', error);

		// Rollback jika ada error
		if (error instanceof Error) {
			return Response.json(
				{
					error: error.message,
					details: 'Failed to process transaction',
				},
				{ status: 500 }
			);
		}

		return Response.json({ error: 'Internal server error' }, { status: 500 });
	}
}
